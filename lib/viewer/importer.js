...
export function openImporter(host, o){
  ...
  list.onclick = async e => {
    ...
    try {
      await o.loadLibrary(d, pct => { if (pct >= 0) bar.firstElementChild.style.transform = `scaleX(${pct / 100})`; });
      close();
    } catch (x){
      bar.hidden = true;
  ...
  async function takeFile(f){
    ...
    try {
      await o.loadFile(f, pct => { if (pct >= 0) bar.style.transform = `scaleX(${pct / 100})`; });
      close();
    } catch (x){
