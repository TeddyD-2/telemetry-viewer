...
export function openChartMenu({ anchor, chart, ctx, types, retype, onChange, actions }){
  ...
  function place(){
    ...
    el.style.top = top + 'px';
    /* Grow out of the settings button, from above it or below it, wherever it ended up. */
    const ox = Math.max(0, Math.min(w, r.left + r.width / 2 - left));
    el.style.transformOrigin = `${ox}px ${top < r.bottom ? '100%' : '0'}`;
