...
export function openChartMenu({ anchor, chart, ctx, types, retype, onChange, actions }){
  ...
  function body(){
    ...
    if (c.type === 'line'){
      ...
      h += row('X axis', seg('x', [['auto', 'Toolbar', 'follow the X axis picker in the toolbar'], ['time', 'Time'], ['dist', 'Distance']]));
      ...
      h += `<div class="cm-sec">Axes</div>`;
      h += row('X channel', `<select data-int="xChan">${chanOpts(c.xChan, c.xChan >= 0 ? null : 'choose…')}</select>`);
      ...
      h += `<div class="cm-sec">Drawing</div>`;
      h += row('Point size', seg('size', [[1, 'S'], [2, 'M'], [3.5, 'L']]));
