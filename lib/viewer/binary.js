...
/* Fetch a cached session, reporting bytes as they arrive so a 20 MB download over a
   ...
   `signal` cancels it. That matters beyond saving bandwidth when someone backs out: two
   downloads of one URL running at once fight over the browser's cache entry, and Chromium
   fails the second with a bare "network error". React mounts the viewer twice in
   development, so without the abort every stored session failed to open under next dev. */
export async function fetchParsed(url, onProgress, signal){
