...
export function createTraces(ctx){
  ...
  function laneKey(){
    const { D } = ctx;
    /* Height is deliberately not in here. Resizing calls setSize on the instance, which
       is smooth; rebuilding every uPlot on each pointermove was not. */
  ...
  function render(root){
    ...
    /* Heights can change without a rebuild, so they are reapplied every render. */
    plots.forEach((p, i) => p && applyHeight(i));
  ...
  function applyHeight(i){
    ...
    if (!p) return;
    const h = ctx.laneHeight(i);
    if (p.height !== h) p.setSize({ width: p.width, height: h });
  }

  /* Drag a lane onto another to overlay them.

     This was HTML5 drag-and-drop, which is the obvious choice and the wrong one here: it
     needs a drag image, it fights uPlot for the pointer inside the plot area, it does
     nothing on touch, and its events cannot be driven from a test. Pointer events do all
     four, and give live feedback while the drag is in flight. */
  function attachLaneDrag(cell, root){
    const { D } = ctx;
    const handle = () => cell.querySelector('.grab');

    cell.addEventListener('pointerdown', e => {
      const h = handle();
      if (!h || !e.target.closest('.grab')) return;   // the plot area belongs to uPlot
      e.preventDefault();
      const from = +cell.dataset.lane;
      cell.classList.add('dragging');
      root.classList.add('dragging-lane');
      let over = null;

      const move = ev => {
        const el = document.elementFromPoint(ev.clientX, ev.clientY);
        const target = el && el.closest('.lane');
        if (target === over) return;
        over?.classList.remove('over');
        over = target && target !== cell ? target : null;
      ...
      const up = () => {
        ...
        root.classList.remove('dragging-lane');
        over?.classList.remove('over');
        const to = over ? +over.dataset.lane : -1;
        if (!(to >= 0) || to === from || !D.lanes[from] || !D.lanes[to]) return;
        const moved = D.lanes[from].filter(ci => !D.lanes[to].includes(ci));
        D.lanes[to] = [...D.lanes[to], ...moved];
        D.lanes.splice(from, 1);
        /* Heights are keyed by lane position, which stops meaning anything the moment
           the lanes are renumbered. */
        D.chartH.clear();
        ctx.onLanesChanged();
      };
  ...
  function addGrip(cell, i){
    ...
    grip.addEventListener('pointerdown', e => {
      ...
      const y0 = e.clientY, h0 = ctx.laneHeight(i);
      const move = ev => {
        D.chartH.set(i, Math.max(90, Math.min(900, h0 + ev.clientY - y0)));
        applyHeight(i);
      };
      const up = () => {
    ...
    grip.addEventListener('dblclick', () => { D.chartH.delete(i); applyHeight(i); });
  }
