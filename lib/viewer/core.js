...
import { openChannelSettings } from './channelSettings.js';
...
export function createViewer(root, opts = {}){
  ...
  const D = {
    ...
    notes:new Map(), notesVer:0,                 // team notes, by channel name
    teamMath:null,                               // the team's shared math channel definitions
  };
  ...
  function busy(msg, pct){
    ...
    if (pct >= 0) $('#prog i').style.transform = `scaleX(${pct / 100})`;
  ...
  /* The last team list seen, so shared channels are still there offline in the paddock. */
  const TEAM_MATH_KEY = 'telemetry-viewer.team-math.v1';
  const isMath = ci => ci >= D.nRaw;
  const mathAt = ci => D.math[ci - D.nRaw];

  try { const c = JSON.parse(localStorage.getItem(TEAM_MATH_KEY) || 'null'); if (Array.isArray(c)) D.teamMath = c; }
  catch { /* no cache */ }

  /* ---- shared with the team: notes and math channels ----
     Both are fetched once when the viewer opens. Either can fail -- offline, a deploy
     without a database -- and the viewer carries on with what this browser has. */
  const api = async (url, init) => {
    const r = await fetch(url, init && { ...init, headers: { 'content-type': 'application/json' } });
    const b = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(b.error || `the server said ${r.status}`);
    return b;
  };
  api('/api/notes').then(b => {
    D.notes = new Map((b.notes || []).map(n => [n.channel, n]));
    D.notesVer++;
    if (D.loaded){ fillAllChannelLists(); render(); }
  }).catch(() => {});
  api('/api/math').then(b => {
    setTeamMath(b.channels || []);
    if (D.loaded) mergeTeamMath(true);
  }).catch(() => {});

  function setTeamMath(list){
    D.teamMath = list;
    try { localStorage.setItem(TEAM_MATH_KEY, JSON.stringify(list)); } catch { /* storage blocked */ }
  }

  /* Team definitions win over a personal one of the same name: the shared version is the
     one everybody else is looking at. */
  function mergeTeamMath(andRender){
    if (!D.teamMath || !D.loaded) return;
    for (const t of D.teamMath){
      const k = D.math.findIndex(m => !m.gone && m.name === t.name);
      if (k >= 0){
        const m = D.math[k];
        Object.assign(m, { expr: t.expr, unit: t.unit, team: true, by: t.createdBy });
        D.units[D.nRaw + k] = t.unit;
      } else if (chanByName(t.name, true) < 0){
        const ci = addMathSlot({ name: t.name, unit: t.unit, expr: t.expr });
        Object.assign(mathAt(ci), { team: true, by: t.createdBy });
      }
    }
    for (const m of D.math) if (m.team && !D.teamMath.some(t => t.name === m.name)) m.team = false;
    if (!andRender) return;
    recomputeMath();
    saveMathLibrary();
    D.notesVer++;
  ...
  async function shareMath(m, from){
    const b = await api('/api/math', { method: 'POST',
      body: JSON.stringify({ name: m.name, unit: m.unit, expr: m.expr, from: from || '' }) });
    m.team = true; m.by = b.channel.createdBy;
    setTeamMath([...(D.teamMath || []).filter(t => t.name !== m.name && t.name !== from), b.channel]);
    saveMathLibrary();
    D.notesVer++;
    fillAllChannelLists(); render();
  }
  async function unshareMath(m, name = m.name){
    await api(`/api/math?name=${encodeURIComponent(name)}`, { method: 'DELETE' });
    m.team = false;
    setTeamMath((D.teamMath || []).filter(t => t.name !== name));
    saveMathLibrary();           // kept, as a personal channel in this browser
    D.notesVer++;
    fillAllChannelLists(); render();
  }

  /* What a channel's tooltip says: what it is, where a math channel comes from, and the
     team's note on it. */
  function tipFor(ci){
    const m = isMath(ci) ? mathAt(ci) : null;
    const lines = [`${D.label[ci]}${D.units[ci] ? ` (${D.units[ci]})` : ''} · ${fmtRange(ci)}`];
    if (m){
      if (m.error) lines.push(`⚠ ${m.error.message}`);
      lines.push(`ƒ ${m.expr}`);
      lines.push(m.team ? `Shared with the team${m.by ? ` by ${m.by}` : ''}` : 'Personal — only in this browser');
    }
    const n = D.notes.get(D.names[ci]);
    if (n) lines.push('', n.note, `— ${n.updatedBy || 'team note'}`);
    return lines.join('\n');
  }

  function openChannel(ci){
    const m = isMath(ci) ? mathAt(ci) : null;
    const name = D.names[ci];
    openChannelSettings(root, {
      label: D.label[ci], unit: D.units[ci], range: fmtRange(ci),
      note: D.notes.get(name) || null,
      math: m && { expr: m.expr, team: !!m.team, by: m.by },
      saveNote: async text => {
        const b = await api('/api/notes', { method: 'PUT', body: JSON.stringify({ channel: name, note: text }) });
        if (b.note) D.notes.set(name, b.note); else D.notes.delete(name);
        D.notesVer++;
        fillAllChannelLists(); render();
        note(b.note ? `Note saved on ${D.label[ci]} for the team` : `Note removed from ${D.label[ci]}`);
      },
      share: on => (on ? shareMath(m) : unshareMath(m)),
      editMath: () => editMath(ci),
    });
  }
  ...
  function saveMathLibrary(){
    try {
      localStorage.setItem(MATH_KEY, JSON.stringify(
        D.math.filter(m => !m.gone && !m.team).map(({ name, unit, expr }) => ({ name, unit, expr }))));
  ...
  function saveMath(ci, full){
    const { shared, ...def } = full;
    const was = ci >= 0 ? { name: mathAt(ci).name, team: !!mathAt(ci).team } : { name: '', team: false };
    if (ci < 0){
      ci = addMathSlot(def);
      recomputeMath();
    ...
    /* Sharing follows the editor's tick box. A shared channel that is edited is edited for
       the team; if the server cannot be reached, the channel is kept in this browser and
       says so rather than silently diverging from the team's copy. */
    const m = mathAt(ci), want = shared ?? was.team;
    if (want){
      shareMath(m, was.team && was.name !== m.name ? was.name : '')
        .then(() => { if (!was.team) note(`${m.name} is now shared with the whole team`); })
        .catch(err => { m.team = false; saveMathLibrary(); note(`Could not share ${m.name}: ${err.message} — kept in this browser`); });
    } else if (was.team){
      unshareMath(m, was.name)
        .then(() => note(`${m.name} is no longer shared — kept in this browser`))
        .catch(err => note(`Could not stop sharing: ${err.message}`));
    }
  }
  ...
  function deleteMath(ci){
    const dm = mathAt(ci);
    if (dm.team){
      if (!confirm(`"${dm.name}" is shared with the team${dm.by ? ` (added by ${dm.by})` : ''}.\n\nDelete it for everyone?`)) return;
      const name = dm.name;
      api(`/api/math?name=${encodeURIComponent(name)}`, { method: 'DELETE' })
        .then(() => setTeamMath((D.teamMath || []).filter(t => t.name !== name)))
        .catch(err => note(`Removed here, but the team copy could not be deleted: ${err.message}`));
      dm.team = false;
    }
  ...
  const charts = createCharts({
    ...
    chanColor, isMath, tipFor: ci => tipFor(ci),
    ...
    if (rebuild || side.dataset.mode !== D.mode){
      ...
      side.querySelectorAll('.panel').forEach(el => {
        ...
        list.onclick = e => {
          ...
          if (e.target.closest('[data-cfg]')){ openChannel(i); return; }
  ...
  function fillChannelList(el, p){
    ...
    const row = i => {
      ...
      const noted = D.notes.has(D.names[i]);
      const cls = ['ch', on ? 'on' : '', D.stats[i].flat && !m ? 'dead' : '',
        m ? 'math' : '', m && m.team ? 'team' : '', m && m.error ? 'err' : '', noted ? 'noted' : ''].filter(Boolean).join(' ');
      return `<div class="${cls}" data-i="${i}" title="${esc(tipFor(i))}">
          <span class="sw" style="${on ? `background:${p.colour(i)}` : ''}"></span>
          <span class="nm">${esc(D.label[i])}</span>
          ${noted ? '<span class="nt" aria-label="has a team note"></span>' : ''}
          <span class="rng">${m && m.error ? '<span class="warn">error</span>' : fmtRange(i)}</span>
          <span class="u">${esc(D.units[i] || '')}</span>
          ${m ? `<button class="ed" data-edit title="edit ${esc(D.label[i])}">✎</button>` : ''}
          <button class="cg" data-cfg title="Channel settings and team note" aria-label="${esc(D.label[i])} settings">
            <svg width="13" height="13" viewBox="0 0 16 16" aria-hidden="true"><path d="M2 4h7M13 4h1M2 8h2M8 8h6M2 12h8" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><circle cx="11" cy="4" r="1.8" fill="none" stroke="currentColor" stroke-width="1.4"/><circle cx="6" cy="8" r="1.8" fill="none" stroke="currentColor" stroke-width="1.4"/><circle cx="12" cy="12" r="1.8" fill="none" stroke="currentColor" stroke-width="1.4"/></svg>
          </button></div>`;
    };
  ...
  /* The hint says what a marker does, which is news once. Repeating it on every M press
     turned a keyboard shortcut into a toast that followed you around. */
  let markerHinted = false;
  function toggleMarker(){
    D.marker = D.marker >= 0 ? -1 : D.cursor;
    if (D.marker >= 0 && !markerHinted){
      markerHinted = true;
      note('Marker set — readouts now show the difference from it. M again clears it.');
    }
    syncChrome(); render();
  }
