...
/* Name matching gets a role most of the way. Where it cannot finish the job, it says
     so rather than picking.

     The Michigan export carries two live channels both called `GPS Speed`, one peaking
     at 79 km/h and one at 284. Any tie-break here -- widest range, first column, closest
     to the GPS track -- is the viewer quietly deciding which of the team's channels is
     real, and being wrong about that is worse than asking: every lap time, the distance
     axis and the cursor readout hang off it. So an ambiguous role resolves to nothing,
     the sidebar opens on it, and the views that need it ask.
  ...
  function resolveRoles(){
    D.role = {}; D.roleAuto = {}; D.roleAmbig = {};
    for (const R of ROLES){
      const c = roleCandidates(R.key);
      const tied = c.filter(x => x.ok && x.rank === c[0]?.rank);
      if (tied.length > 1){
        D.roleAmbig[R.key] = tied.map(x => x.i);
        D.role[R.key] = D.roleAuto[R.key] = -1;
        continue;
      }
    ...
    if (root.dataset.k !== need){
      ...
      D.sel.forEach((ci,k) => {
        ...
        d.querySelector('.x').onclick = e => { e.stopPropagation();
          D.sel = D.sel.filter(x => x !== ci); fillAllChannelLists(); render(); };
        attachResize(d, ci);
    ...
    if (rebuild || side.dataset.mode !== D.mode){
      ...
      side.querySelectorAll('.panel').forEach(el => {
        ...
        el.querySelector('[data-clear]')?.addEventListener('click', () => {
          D.sel = []; renderSidebar(true); render();
        });
