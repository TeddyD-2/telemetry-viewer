...
export function createViewer(root, opts = {}){
  ...
  /* Adding a ninth channel used to silently drop the first chart -- the one most likely
     to be the reference everything else was being read against. There is still a limit,
     because every chart is a live canvas the cursor redraws, but it is well past what fits
     on a screen, and reaching it says so instead of taking something away.

     Heights are keyed by the channels a chart holds (see traces.js), so toggling one
     channel no longer resets the height of every other chart. */
  ...
  function toggleChannel(ci){
    ...
    if (flatSel().length >= MAX_PLOTTED){
      note(`${MAX_PLOTTED} channels is the most one stack will plot — remove one first`);
      return false;
    }
    D.lanes.push([ci]);
    return true;
  ...
  /* A one-line message that goes away on its own, for things worth saying but not worth
     a dialog. */
  let noteTimer = 0;
  function note(msg){
    let el = $('#toast');
    ...
    noteTimer = setTimeout(() => el.classList.remove('on'), 3400);
  }
  ...
  function ingest(m, fname){
    ...
    D.cursor = 0;
    $('#layoutmenu').hidden = D.mode !== 'traces';
  ...
  function setRole(key, i){
    ...
    if (key === 'speed' || key === 'lat' || key === 'lon' || key === 'dist'){
      ...
      /* `dist` is one of the things an expression can use. */
  ...
  /* A math channel is appended to the session's own columns, so every view takes it like
     a logged channel: a trace, the map colour, the histogram, the CSV export. The
     definitions -- name, unit, expression -- are kept in this browser and applied to
     every session opened, which is how RS3 treats them too: wheel slip is written once
     per car, not once per file. One that refers to a channel this file does not have
     stays in the list with its error, rather than quietly disappearing. The expression
     language itself is in math.js. */
  ...
  function applyMathLibrary(){
    ...
    for (const d of loadMathLibrary()){
      if (chanByName(d.name, true) >= 0) continue;      // this file logs a channel by that name
      addMathSlot(d);
    }
    recomputeMath();
  }
  ...
  /* How an expression's names find channels: the label as shown in the sidebar (so a
     duplicated name is reachable as "GPS Speed #2"), then the raw name, which resolves
     a duplicate to the live one. A bare `g` or `time` is the constant; only after that
     does a case-insensitive match get a look in. */
  ...
  /* Evaluate every definition, each one after the math channels it uses. */
  ...
  /* The editor's live preview: the same evaluation, without committing anything. */
  ...
  /* ci of an existing math channel, or -1 for a new one. */
  ...
  function saveMath(ci, def){
    if (ci < 0){
      ...
      if (!mathAt(ci).error){
        if (D.mode === 'traces') toggleChannel(ci);
        else {
          const key = { track: 'mapChan', compare: 'cmpChan', analysis: 'histChan' }[D.mode];
          if (key) D[key] = ci;
        }
      }
    } else {
  ...
  function deleteMath(ci){
    if (D.lanes.some(l => l.includes(ci))) toggleChannel(ci);
    mathAt(ci).gone = true;
    D.gone.add(ci);
    D.names[ci] = D.label[ci] = '';
    D.cols[ci] = new Float64Array(D.n).fill(NaN);
    const fallback = roleIdx('speed') >= 0 ? roleIdx('speed') : 1;
    for (const key of ['mapChan', 'cmpChan', 'histChan']) if (D[key] === ci) D[key] = fallback;
    recomputeMath();
    saveMathLibrary();
    renderSidebar(true); render();
  }
  ...
  const traces = createTraces({
    D,
    xArr, xFmt, fmtD, esc,
    chanColor, isMath, note,
    maxPlotted: MAX_PLOTTED,
    laneHeight: id => D.chartH.get(id) || D.traceH,
    setView(range){
      const xs = xArr();
  ...
  function renderTrack(root){
    ...
    /* Colour covers the racing, not the whole file.

       A session starts with the car being pushed to grid and rolling out of the paddock,
       and ends the same way. Those samples are on the map but they are not laps, and
       including them in the ramp's range spends most of the gradient on a crawl -- every
       actual lap then lands in the top third of the ramp and reads as flat, which is the
       opposite of what the map is for. Anything outside the timed laps is drawn as
       context in grey and left out of the range entirely. */
    const first = D.laps.length ? D.laps[0].i0 : 0;
    const last = D.laps.length ? D.laps[D.laps.length-1].i1 : D.n-1;
    const a0 = lap ? lap.i0 : first, a1 = lap ? lap.i1 : last;
    ...
    /* Two passes, context first. The car covers the same tarmac 22 times, so drawing the
       coloured section inline with everything else means later grey paints straight over
       it and the selection vanishes. */
    g.lineWidth = 2.4; g.lineCap = 'round';
    const drawSpan = (from, to, stroke) => {
      if (to <= from) return;
    ...
    drawSpan(1, a0, '#2c2c29');
    drawSpan(a1, D.n-1, '#2c2c29');
    if (lap) drawSpan(first, last, '#2c2c29');
    ...
    if (rebuild || side.dataset.mode !== D.mode){
      ...
      side.querySelectorAll('.panel').forEach(el => {
        ...
        list.onclick = e => {
          ...
          if (traces.justDragged()) return;
          const i = +row.dataset.i;
          /* A math channel with an error has nothing to plot; the useful click is the
             one that opens it to be fixed. */
        ...
        /* In Traces a row can also be dragged straight onto the stack: between charts for
           a chart of its own, onto one to overlay it there. */
        list.onpointerdown = D.mode === 'traces' && p.pick === 'multi' ? e => {
          const row = e.target.closest('.ch');
          if (!row || e.target.closest('button')) return;
          const i = +row.dataset.i;
          if (isMath(i) && mathAt(i).error) return;
          traces.pressChannel(e, i);
        } : null;
      });
      bindRolesPanel();
    }
  ...
  function fillChannelList(el, p) {
    ...
    D.names.forEach((n, i) => {
      ...
      /* Math channels are listed first and never hidden as flat: someone wrote them. */
  ...
  /* Menus. Native <select> covers View and X axis; these two hold mixed controls, so
     they are popovers -- still one open at a time, still closed by Escape or a click
     anywhere else, which is the part people notice when it is missing. */
  ...
  on(window, 'resize', () => { clearTimeout(rt);
    /* The trace stack sizes itself (a ResizeObserver per chart); tearing it down here reset
       the scroll position on every resize. The canvas views still need a redraw. */
