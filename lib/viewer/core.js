...
export function createViewer(root, opts = {}){
  ...
  traceH:210, traceCols:1, chartH:new Map(),   // trace layout: default height, tiling, per-chart overrides
  ...
  function renderTraces(root){
    ...
    const need = D.sel.map(c => 'tr'+c).join(',') + '|' + D.traceCols;
    if (root.dataset.k !== need){
      root.dataset.k = need;
      root.innerHTML = '';
      ...
      D.sel.forEach((ci,k) => {
        ...
        d.querySelector('.x').onclick = e => { e.stopPropagation();
          D.sel = D.sel.filter(x => x !== ci); renderSidebar(); render(); };
        attachResize(d, ci);
    ...
    [...root.children].forEach((d,k) => {
      const H = D.chartH.get(+d.dataset.ci) || D.traceH;
  ...
  /* Drag the bottom edge of a lane to give it more room. Dragging one lane sets that
     lane only, because the reason to enlarge one is that it is the channel being read;
     the rest should stay where they are. Double-click puts it back to the default. */
  function attachResize(d, ci){
    const grip = d.querySelector('.grip');
    if (!grip) return;
    grip.onpointerdown = e => {
      e.preventDefault();
      const y0 = e.clientY, h0 = D.chartH.get(ci) || D.traceH;
      grip.setPointerCapture(e.pointerId);
      const move = ev => {
        D.chartH.set(ci, Math.max(90, Math.min(900, h0 + ev.clientY - y0)));
        render();
      };
      const up = () => { grip.onpointermove = null; grip.onpointerup = null; };
      grip.onpointermove = move;
      grip.onpointerup = up;
    };
    grip.ondblclick = () => { D.chartH.delete(ci); render(); };
  }
  ...
  const SIDEBAR = {
    traces:   [{ kind: 'channels', pick: 'multi', title: 'Channels', grow: true },
               { kind: 'laps' }],
    compare:  [{ kind: 'laps', grow: false },
               { kind: 'reflap' },
               { kind: 'channels', pick: 'single', title: 'Compare channel',
                 get: () => D.cmpChan, set: v => { D.cmpChan = v; }, grow: true }],
    track:    [{ kind: 'channels', pick: 'single', title: 'Colour the map by',
                 get: () => D.mapChan, set: v => { D.mapChan = v; }, grow: true },
               { kind: 'laps' }],
    analysis: [{ kind: 'channels', pick: 'single', title: 'Histogram of',
                 get: () => D.histChan, set: v => { D.histChan = v; }, grow: true },
               { kind: 'channels', pick: 'multi', title: 'Channels in the table' },
               { kind: 'laps' }],
  };

  function panelHTML(p, idx){
    const grow = p.grow === false ? 'flex:none' : (p.grow ? 'flex:1' : 'flex:none');
    if (p.kind === 'laps'){
      return `<div class="panel" style="${p.grow ? 'flex:1' : 'flex:none'}">
        <h3>Laps <span class="n" id="lapn"></span></h3>
        <div class="body" id="lapwrap"></div></div>`;
    }
    if (p.kind === 'reflap'){
      return `<div class="panel" style="flex:none">
        <h3>Reference lap</h3>
        <div class="ctl"><select id="refsel"></select></div></div>`;
    }
    return `<div class="panel chanpanel" data-p="${idx}" style="${grow}">
      <h3>${p.title} <span class="n" data-count></span></h3>
      ...
      <div class="body chanwrap"></div></div>`;
  }

  /* Rebuilt only when the mode changes -- a rebuild would drop the search box's focus
     and its text mid-keystroke otherwise. */
  function renderSidebar(){
    const side = $('#side');
    const panels = SIDEBAR[D.mode] || SIDEBAR.traces;
    if (side.dataset.mode !== D.mode){
      side.dataset.mode = D.mode;
      side.innerHTML = panels.map(panelHTML).join('') + rolesPanelHTML();
      side.querySelectorAll('.chanpanel').forEach(el => {
        ...
        q.oninput = () => { D.chQuery = q.value; renderSidebar(); };
        flat.onchange = () => { D.hideFlat = flat.checked; renderSidebar(); };
        el.querySelector('[data-clear]')?.addEventListener('click', () => {
          D.sel = []; renderSidebar(); render();
        });
        el.querySelector('.chanwrap').onclick = e => {
          ...
          if (p.pick === 'single'){ p.set(i); }
            ...
            if (k >= 0) D.sel.splice(k, 1);
            else { if (D.sel.length >= 8) D.sel.shift(); D.sel.push(i); }
          }
          renderSidebar(); render();
        };
      });
    ...
    side.querySelectorAll('.chanpanel').forEach(el => fillChannelList(el, panels[+el.dataset.p]));
  ...
  function fillChannelList(el, p) {
    ...
    D.names.forEach((n, i) => {
      const chosen = single ? i === cur : D.sel.includes(i);
    ...
    el.querySelector('[data-count]').textContent =
      `${shown} / ${D.names.length}` + (single ? '' : ` \u00b7 ${D.sel.length} plotted`);
    ...
    for (const [gn, list] of [...groups].sort((a, b) => b[1].length - a[1].length || a[0].localeCompare(b[0]))){
      ...
      for (const i of list){
        const k = single ? (i === cur ? 0 : -1) : D.sel.indexOf(i);
        const colour = single ? 'var(--brand)' : SLOT[k % 8];
  ...
  function rolesTrouble(){
    ...
    const ambiguous = ROLES.filter(R => {
      const i = roleIdx(R.key);
      return i >= 0 && D.dupe[D.names[i]];
    });
