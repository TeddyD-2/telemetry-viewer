...
export function createCharts(ctx){
  ...
  function makeView(id, fresh){
    ...
    cell.addEventListener('transitionend', () => cell.classList.remove('enter'));
    return v;
  }
  ...
  function placeBar(v, kind){
    ...
    if (columns() > 1 && !v.cell.classList.contains('wide')){
      ...
      Object.assign(b.style, { transform: `translate(${x - wr.left - T / 2}px, ${r.top - wr.top}px)`, width: `${T}px`, height: `${r.height}px` });
    } else {
      const y = kind === 'before' ? r.top - gap / 2 : r.bottom + gap / 2;
      Object.assign(b.style, { transform: `translate(${r.left - wr.left}px, ${y - wr.top - T / 2}px)`, width: `${r.width}px`, height: `${T}px` });
    }
