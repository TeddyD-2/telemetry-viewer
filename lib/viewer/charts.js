...
export function createCharts(ctx){
  ...
  function headSig(c){
    return [c.type, c.chans.join('+'), c.xChan, c.overlays && ctx.sessSig(), c.stats,
  ...
  TYPES.xy = {
    sig: c => ['xy', verOf(c.xChan), c.chans.map(verOf).join('+'), c.colorBy >= 0 ? verOf(c.colorBy) : '-',
      c.range, c.fit, c.equal, c.rings, c.size, c.overlays && ctx.sessSig()].join('|'),
  ...
  function drawPoints(u, v, c){
    ...
    for (const s of st.sets){
      if (s.primary && st.color){
        ...
        g.fillStyle = s.primary ? withAlpha(s.color.startsWith('#') ? s.color : '#888888', .6) : s.color;
