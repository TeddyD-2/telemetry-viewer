...
/* Shared sessions, plus any unlisted ones this browser uploaded. */
export function listDatasets(sql, viewerToken){
  return viewerToken
    ? sql`select * from datasets
          where listed = true or owner_token = ${viewerToken}
...
/* `viewerToken` is the token the requesting browser sent. The token itself is never
   echoed back -- knowing someone else's would be enough to edit their session -- so the
   comparison happens here and only the answer travels. */
export function rowToDataset(r, viewerToken){
  return {
    id: r.id,
    mine: !!viewerToken && r.owner_token === viewerToken,
    title: r.title,
